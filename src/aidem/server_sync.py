...
~/.aidem/org/<org>/registry/<kind>/<name> and links them into the shared
...
from .paths import kind_dir, org_registry_dir, org_root
...
def _is_org_owned(target: Path, org: str) -> bool:
    """True when `target` is part of this org's linked content.

    Org skill links are dirs containing a SKILL.md symlink into the org
    clone; org rule links are direct symlinks. Real files/dirs are personal
    content and never count as org-owned.
    """
    root = org_root(org).resolve()
    ...
    if target.is_symlink():
        return under_root(target)
    if target.is_dir():
        skill_md = target / "SKILL.md"
        return skill_md.is_symlink() and under_root(skill_md)
    return False
...
def _mcp_entries_for(clone: Path) -> dict[str, dict]:
    ...
    if root_file.exists():
        ...
        mcp_sub = clone / "mcp"
        files = sorted(mcp_sub.glob("*.json")) if mcp_sub.is_dir() else []
...
def sync_org_content(echo=print) -> tuple[int, int, int]:
    """Clone/pull org content, refresh links + MCP configs, prune stale entries.
    ...
    state = server_state.load_state()
    org = state["org"]
    sources = [s for s in server_state.org_content_sources()
               if s.get("name") and s.get("git_url")]
